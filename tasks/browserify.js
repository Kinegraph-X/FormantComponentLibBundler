...
module.exports = function(grunt, options) {
	return {
		debug: {
			files: {
				'<%=pathToProject%>/build/<%=currentProject%>.js' : [
					options.basePath + '/main.js'
				]
			},
			options: {
				browserifyOptions: {
...
////					console.log(grunt.config.data.UIvalidatorsList);
//					browserifyInstance.require(grunt.config.data.UIPackageList);
//					browserifyInstance.require(grunt.config.datUIvalidatorsList);
//				},
...
//					['highlighted-sources', {pathToProject : '<%=pathToProject%>', launcherFilePath : '<%=launcherFilePath%>'}]
				]
			}
		},
		release: {
